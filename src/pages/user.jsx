...
const UserManagement = () => {
  ...
  const validateForm = () => {
    ...
    if (editingUserEmail) {
      if (!editingUserEmail || !emailRegex.test(editingUserEmail))
    ...
    if (!editingUserEmail && (!password || password.length < 6))
  ...
  const handleDeviceAssignment = async (device, assign = true) => {
    try {
      const response = await fetch(
        "http://13.201.156.32:5000/api/assign-devices",
        {
          method: "POST",
          headers: {
      ...
      if (assign && data.message === "Devices assigned successfully") {
        const assignedDevice = data.results.find(
          (r) => r.device_id === device.device_id,
        );
        if (
          assignedDevice &&
          assignedDevice.status === "assigned successfully"
        ) {
          return true;
        }
      } else if (
        !assign &&
        data.message === "Devices unassigned successfully"
      ) {
        const unassignedDevice = data.results.find(
          (r) => r.device_id === device.device_id,
        );
        ...
        data.message === "No devices were assigned" &&
      ...
      setError(
        `Failed to ${assign ? "assign" : "unassign"} device: ${error.message}`,
      );
      return false;
    }
  };
  ...
  const saveDevices = async () => {
    ...
    try {
      const currentRes = await API.get(
        `/user?email=${encodeURIComponent(devicesView.email)}`,
      );
      const currentDevices = currentRes.data.devices || [];

      const devicesToAdd = editingDevices.filter(
        (ed) => !currentDevices.some((cd) => cd.device_id === ed.device_id),
      );

      const devicesToRemove = currentDevices.filter(
        (cd) => !editingDevices.some((ed) => ed.device_id === cd.device_id),
      );
      ...
      setEditingDevices([]);
      setDevicesModified(false);
          toast.success("Devices saved successfully ✅");  // 👈 ADD THIS
  ...
  const handleSaveDeviceEdit = async () => {
    ...
    const response = await API.put(`/device/${editingDevice}`, {
      device_name: deviceEditData.device_name, // ✅ FIXED
    ...
    if (response.data.success) {
          toast.success("Device updated successfully ✅");  // 👈 ADD THIS
      ...
      setEditingDevices((prev) =>
        prev.map((device) =>
          device.device_id === editingDevice
            ? {
                ...device,
                device_name: deviceEditData.device_name, // ✅ UPDATE HERE ALSO
                device_location: deviceEditData.device_location,
                device_site_name: deviceEditData.device_site_name,
                latitude: deviceEditData.latitude,
                longitude: deviceEditData.longitude,
  ...
  const handleAddNewDevice = async () => {
    ...
    try {
      ...
      const deviceExists = allDevices.some(
        (d) => d.device_id === addNewDeviceData.device_id,
      );
      ...
      setError(
        err.response?.data?.message || "Failed to add and assign new device.",
      );
    } finally {
      setIsAssigningDevices(false);
    }
  ...
  const availableDevices = allDevices.filter(
    (device) => !editingDevices.some((d) => d.device_id === device.device_id),
  );
                ...
                return (
                  <div className="input-group" key={key}>
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      required={
                        !["password", "new_email"].includes(key) &&
                        !(key === "email" && editingUserEmail !== null)
                      }
                      disabled={key === "email" && editingUserEmail !== null}
                      placeholder={
                        key === "email" && editingUserEmail ? "" : undefined
                      }
                    />
                  </div>
                );
                ...
                <td>
                  <span
                    className={`status-indicator ${
                      new Date(user.expiring_date) >= new Date()
                        ? "active"
                        : "suspended"
                    }`}
                    title={
        ...
        {viewUser && (
          ...
          >
            ...
            <div className="user-details-grid">
              ...
              <div>
                <strong>Password:</strong> {viewUser.password || "(hidden)"}
              </div>
              <div>
                <strong>Client Name:</strong> {viewUser.client_name}
              </div>
              <div>
                <strong>Mobile No:</strong> {viewUser.mobile_no}
              </div>
              <div>
                <strong>Lat/Long:</strong> {viewUser.lat_long}
              </div>
              <div>
                <strong>Activation Date:</strong>{" "}
                {viewUser.activation_date?.slice(0, 10)}
              </div>
              <div>
                <strong>Expiring Date:</strong>{" "}
                {viewUser.expiring_date?.slice(0, 10)}
              </div>
              <div>
                  ...
                  <ul>
                    {viewUser.devices.map((device) => (
                      <li key={device.device_id}>
                        {device.device_id} (
                        {getDeviceDisplayName(device.device_name || "unknown")})
                        {device.device_location && (
                          <div>Location: {device.device_location}</div>
                        )}
                        {device.device_site_name && (
                          <div>Site: {device.device_site_name}</div>
                        )}
                        {device.latitude && device.latitude !== "NA" && (
                          <div>Latitude: {device.latitude}</div>
                        )}
                        {device.longitude && device.longitude !== "NA" && (
                        ...
                        {editingDevice === device.device_id ? (
                          <div className="device-edit-form">
                            <div className="input-group">
                              ...
                              <select
                                name="device_name"
                                value={deviceEditData.device_name}
                  ...
                  <ul className="available-devices-list">
                    {filteredAvailableDevices.map((device) => (
                      ...
                      >
                        ...
                        {selectedAvailableDevices.includes(
                          device.device_id,
                ...
                {showAddNewDeviceForm && (
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={
                        isAssigningDevices ||
                        !addNewDeviceData.device_id.trim() ||
                        !addNewDeviceData.device_name
                      }
